...
sed -i "s/\/home\/binary\/womginx\/public/\/opt\/womginx\/public/g" /etc/nginx/nginx.conf
...
proxy_set_header Accept-Encoding/" /etc/nginx/nginx.conf
...
sed -i "s/binary_remote_addr/http_x_forwarded_for/g" /etc/nginx/nginx.conf

# pipe logs to stdout/stderr instead of the default /var/log/nginx/ location
sed -i 's/http {/http {access_log \/dev\/stdout; error_log stderr;/' /etc/nginx/nginx.conf
